...
const createReducer = (initialState: SectionsState) =>
	reducerWithInitialState(initialState)
		.case(addSection, (state, section) => ({
			...state,
			items: [...state.items, section],
		}))
		...
		.case(splitSection, (state, id) => {
			...
			return {
				...state,
				items: [...state.items.filter((section) => section.id !== id), newContainer, newSection1, newSection2],
			}
		})
		...
		.case(updateSectionItem, (state, item) => {
			...
			return {
				...state,
				items: state.items.map((section) => (section.id === item.id ? item : section)),
			}
		})
		...
		.case(switchSectionItemPin, (state, { id, itemId, pinned: pinnedState }) => {
			const section = state.items.find((section) => section.id === id) as
				| SectionItemHistory
				| SectionItemVCS
				| SectionItemITS
			...
			return {
				...state,
				items: state.items.map((section) =>
					section.id === id ? ({ ...section, pinned, items } as unknown as typeof section) : section,
				),
			}
		})
		.case(updatePinnedItems, (state, { id, items }) => {
			...
			const pinned = [...(section.pinned ?? [])].map((item) => {
			...
			return {
				...state,
				items: state.items.map((section) =>
					section.id === id ? ({ ...section, pinned } as unknown as typeof section) : section,
				),
			}
		})
