...
function generateVCSName(): string {
	return `${faker.lorem.word({ length: { min: 2, max: 5 } })}/${faker.lorem.slug({ min: 2, max: 3 })}`
}
function generateITSName(): string {
	return `${faker.lorem.word({ length: { min: 2, max: 4 } }).toUpperCase()}-${faker.number.int({
		min: 100,
		max: 10000,
	})}`
}
