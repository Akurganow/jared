...
export default function HistorySection({ id, withTitle, className, ...props }: SectionProps) {
	...
	const handlePinClick = useCallback(
		({ itemId, pinned }: { itemId: string; pinned: boolean }) =>
			(event: MouseEvent) => {
	...
	const items = [...(section.items ?? [])]
		.filter((item) => !pinned.find((pinnedItem) => pinnedItem.id === item.id))
	...
	return (
		...
		>
			{pinned.map(({ id: itemId, title, url }) => (
				<a href={url} key={itemId + title} className={cn(st.item)} title={title}>
					{url && <Favicon href={url} size={16} />}
					<div className={st.text}>{title}</div>
					<PinButton className={st.pinButton} pinned={true} onClick={handlePinClick({ itemId, pinned: true })} />
				</a>
			))}
			{items.map(({ id: itemId, title, url }) => (
				<a href={url} key={itemId + title} className={cn(st.item)} title={title}>
					{url && <Favicon href={url} size={16} />}
					<div className={st.text}>{title}</div>
					<PinButton className={st.pinButton} pinned={false} onClick={handlePinClick({ itemId, pinned: false })} />
				</a>
			))}
		</SectionItem>
	)
